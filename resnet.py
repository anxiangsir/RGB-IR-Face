...
def residual_unit_v1(data, num_filter, stride, dim_match, name, bottle_neck, use_global_stats, **kwargs):
...
def residual_unit_v1_L(data, num_filter, stride, dim_match, name, bottle_neck, use_global_stats, **kwargs):
    """Return ResNet Unit symbol for building ResNet
    Parameters
    ----------
    data : str
...
def residual_unit(data, num_filter, stride, dim_match, name, bottle_neck, use_global_stats, **kwargs):
    uv = kwargs.get('version_unit', 3)
    version_input = kwargs.get('version_input', 1)
    if uv == 1:
        if version_input == 0:
            return residual_unit_v1(data, num_filter, stride, dim_match, name, bottle_neck, use_global_stats, **kwargs)
        else:
            return residual_unit_v1_L(data, num_filter, stride, dim_match, name, bottle_neck, use_global_stats,
                **kwargs)


def resnet(data, units, num_stages, filter_list, num_classes, bottle_neck, use_global_stats, **kwargs):
    bn_mom = kwargs.get('bn_mom', 0.9)
    workspace = kwargs.get('workspace', 256)
    ...
    fc1 = mx.sym.BatchNorm(fc1, fix_gamma=False, eps=2e-5, momentum=bn_mom, name='fc1')
    fc1 = mx.sym.L2Normalization(fc1, name='fc1_norm')
    return fc1


def get_symbol(data, num_classes, num_layers, use_global_stats, **kwargs):
    """
